...
<!--      <xsl:when test="$elt/self::dbk:part[jats:is-appendix-part(.)]"><xsl:sequence select="'app-group'"/></xsl:when>-->
  ...
  <xsl:template match="dbk:bibliography | dbk:glossary | dbk:appendix | dbk:index |  dbk:preface[@role = 'acknowledgements'][preceding-sibling::*[self::dbk:appendix | self::dbk:chapter |  self::dbk:part]] |
